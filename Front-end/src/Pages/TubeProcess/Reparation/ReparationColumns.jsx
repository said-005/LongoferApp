...
import { useDeleteReparation } from "./deleteReparationHook";
import { UpdateSheet } from "../../Shette";
import UpdateReparation from "./updateReparation";

export const ReparationColumns = [
  {
    accessorKey: "code_Reparation",
    header: "Code Réparation",
    cell: ({ row }) => (
      <div className="font-medium uppercase text-xs sm:text-sm">
        {row.getValue("code_Reparation") || '-'}
      </div>
    ),
  },
  {
    ...
    header: "Date",
    cell: ({ row }) => {
      const date = row.getValue("date_reparation");
      return (
  ...
  },
  {
    ...
    header: '',
    cell: ({ row }) => {
      const reparation = row.original;
      const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
      const { mutate: deleteReparation, isPending: isDeleting } = useDeleteReparation();

      const handleDelete = () => {
        deleteReparation(reparation.code_Reparation, {
          onSuccess: () => {
            ...
            <AlertDialogContent className="max-w-[95%] sm:max-w-md">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="text-xs sm:text-sm">
                  Cette action supprimera définitivement cet enregistrement de réparation et ne peut pas être annulée.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
