...
export default function SablageEXTList() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: SablageEXTApi.getAll,
    onError: (error) => {
      toast.error("Échec du chargement des Sablage Externes", {
    ...
    <div className="container mx-auto px-2 sm:px-4 py-4 md:py-6 mt-4 md:mt-20">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 md:mb-6 gap-2 md:gap-4">
        <div className="text-center md:text-left">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">Liste des Sablage Externe</h1>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">
            Gestion des Sablage Externe
          </p>
        </div>
        <Link 
          to={'/sablage_ext/addSablage_ext'} 
        ...
        >
          <span className="hidden sm:inline">+ Nouvelle Sablage EXT</span>
